...
\author{Deepak Gopinath$^{1,3}$, Siddarth Jain$^{2,3}$ and Brenna D. Argall$^{2,3}$% <-this % stops a space
...
\begin{document}
	...
	challenge---for example, due to their physical impairments or limitations of the control interfaces. Such interfaces often issue control signals that are low-dimensional, discrete and operate in different modes, each of which correspond to different parts of the control space that must be switched between. The introduction of
	...
	can provide good starting points but may not remain optimal for the user in the long term. For example, the subject's abilities will likely be changing---either degrading (e.g.~due to degenerative disease) or improving (e.g.~due to successful rehabilitation). As a result of which the need for assistance may be increased or decreased. One way to accomplish customization is to tune the system parameters which will bring about a change in the human-robot interaction and the final behavior. The aim is to optimize the human-robot interaction during task performance. A simple choice of optimality criterion is to consider task-related performance metrics such as minimizing the time taken and energy expended. 
	
	Our insight is that if we entrust the task of customization to the users, they likely will tune the system in such a way that the optimal interaction---according to their personal optimality criterion---will emerge. Moreover, the user-driven customization of assistance may be task-dependent in addition to being user-dependent. 
	...
	as an object)~\cite{jain2010assistive} or pose correction~\cite{04ar-bien}, with the robot autonomously generating the motion commands. 
	...
	Optimization techniques have been adopted to generate different strategies for control sharing; for example, formulating the problem as a POMDP and inferring a distribution over goals from user commands via Maximum Entropy Inverse Optimal Control~\cite{javdani2015shared}, using pseudo-navigation functions for collaborative control in robotic wheelchairs~\cite{fernandez2015towards} or concatenating energy-optimal motion primitives to create optimal trajectories~\cite{lawitzky2013trajectory}. Although these approaches result in improved task performance (task completion time, control effort), the assistance schemes are mixed in terms of user acceptance. In particular, there are instances of assistance resulting in higher user dissatisfaction~\cite{javdani2015shared}, and users preferring to be in control and more cautious~\cite{lawitzky2013trajectory}. In other studies users find the assistance at times to be uncooperative and are willing to tolerate a loss of control only when there is a significant improvement in performance~\cite{you2012assisted}.
	...
	In an attempt to construct more realistic cost functions, other researchers incorporate a measure of ``discomfort" into the optimization cost function. Inspired by research in design (transportation, elevator etc.) a discomfort cost function is formulated~\cite{gulati2009framework}, however, the specific form of the cost function is domain dependent and is not generalizable to other assistive devices such as robotic arms. 
	%Trajectory generation have been explored in Experimental evaluation revealed that with the proposed scheme the human effort required to accomplish a task was drastically reduced, however the subjects became more cautious and opted for higher execution times as the optimal trajectories ran really close to the walls and corners of the virtual maze in which the scheme was tested. 
	
	Despite an improvement in task performance, none of the above cost function formulations were able to guarantee high user satisfaction (with the exception of domain-specific~\cite{gulati2009framework}). The need for higher user satisfaction is crucial for the acceptance of robot autonomy by the end-users in the assistive domain. This gap motivates our approach to engage the end-user in the optimization procedure.
	...
	The control signal from the robot autonomy is generated by a function $f(\cdot) \in \mathcal{F}$, 
	\begin{equation}
		\boldsymbol{u}_r(t) \leftarrow f(\boldsymbol{x}(t))
	\end{equation}
	where $\mathcal{F}$ is the set of all control behaviors corresponding to different tasks.
	
	We assume that the control command $\boldsymbol{u}_h(t)$ is generated by a function of  $g(\cdot) \in \mathcal{G}$,
	\begin{equation}
		\boldsymbol{u}_h(t) \leftarrow g(\boldsymbol{x}(t))
	\end{equation}
	where $\mathcal{G}$ is the set of user behaviors corresponding to different tasks. $g(\cdot)$ is simply a symbolic representation of the mapping function that generates $\boldsymbol{u}_{h}(t)$ and is completely unknown to the autonomous system.
	The sole dependence of $\boldsymbol{u}_h(t)$ on $\boldsymbol{x}(t)$ is an approximation because there may be a large number of unobserved variables (e.g~fatigue or personal satisfaction) affecting the user's decision. 
	...
	\end{equation}
	where $\boldsymbol{a}(\cdot)$ is in general a non-linear, time--varying function. Note that in this formulation the parameters $\boldsymbol{\theta}(t)$ are treated in the \textit{same way} as the other control signals.
	...
	Optimal control models assume the existence of some kind of cost function being optimized during task performance.\footnote{For example, arm trajectories generated by uninjured humans during reaching tasks have been reproduced using a cost function composed of total torque generated at the joints~\cite{uno1989formation} or total jerk of the end effector (hand)~\cite{flash1985coordination}.} In general, the cost function $\boldsymbol{J}$ can be written as,  
	\begin{equation}
	...
	\end{equation}
where the first term corresponds to a terminal cost (e.g~proximity of the end pose to the target pose) and the second term corresponds to a measure of internal cost (e.g~the energy expended during task execution, task completion time, etc). The true cost function, however likely is more complex, and could include additional factors such as user satisfaction. The state boundary conditions are given by 
	$
	...
	The elements of the framework $f(\cdot)$, $g(\cdot)$, $\beta(\cdot)$ and 
	...
	form pictured in Figure~\ref{PAF} \cite{dragan2012formalizing}. This particular function is characterized by a set of three parameters $\{\theta_{1}, \theta_{2}, \theta_{3}\}$ and independent variable $c(t)$. The parameter set determines:
	\begin{itemize}
	...
	The arbitrated signal $\boldsymbol{u}(t)$ was the final velocity of the end-effector in Cartesian space, converted to joint-space velocities via inverse kinematics.
	%\vspace{-0.22cm}
	...
	agreement between the user-generated commands and robot-generated commands. 
	...
	task-specific weights.\footnote{
		Prior to computing $c(t)$ the control commands $\boldsymbol{u}_{h}(t)$ and $\boldsymbol{u}_{r}(t)$ are smoothed using a moving average filter ($0.6s$), so that small command changes do not affect the confidence measure drastically.}
	
	At each execution step this confidence measure is computed for all
	candidate goals in the scene, $\lambda \in \Lambda$, resulting in a
	%. Thus, we have a
	distribution of confidences $c_\lambda(t) \in \mathcal{C}$
	%, \forall g \in \mathcal{G}$ 
	over the candidate goals.\footnote{In this study the candidate goals are objects placed at predefined positions in front of the robot. Our system is also able to autonomously perceive object positions and use these as candidate goals.} To compute these confidences,
	each control behavior $f_\lambda$ generates a command (where $f_\lambda$ aims to
	reach candidate goal $\lambda$) which is used in the calculation of
	$c_\lambda(t)$ according to (\ref{eq:conf}).
	...
	\begin{figure*}[t]
		\centering
		\includegraphics[width = 1\hsize]{./finalfigures/Figure3.png}
		\caption{Study tasks  performed by an spinal cord injury (SCI) participant. \textit{Left to right:} Simple Reaching (R), Reaching for Grasping (RfG), Reaching for Scooping (RfS).}
		\label{TOne}
	\end{figure*}
	...
	arm. We refer to this as the \textit{2D mapping paradigm}. The second uses all three of the joystick axes under two 3D operational modes and is referred to as the \textit{3D mapping paradigm}. 
	...
	derive the behavior functions in $\mathcal{F}$, there are some
	limitations on the form that $f$ should take. Attempting to return the robot to the
	...
	Our current implementation favors
	dynamical systems formulations.
	The autonomous robot policies are derived from human demonstrations using a Learning from Demonstration approach known as \textit{Stable Estimator of Dynamical Systems (SEDS)} \cite{khansari2011learning}. In SEDS, the target poses are modeled as attractors of a dynamical system. 
	For each task, a set of $N$ demonstrations are collected by
	...
	\begin{equation}
		\boldsymbol{\dot{x}}(t) \leftarrow f(\boldsymbol{x}(t))
	\end{equation}
	where $\boldsymbol{u}_{r}(t) \triangleq K(\boldsymbol{\dot{x}}(t))$ and $K(\cdot)$ is the forward kinematics of the robot arm (since human teleoperation and control blending both happen in the  end-effector Cartesian space). The dynamical system ensures that the policy exists everywhere in the workspace, and that the robot trajectories follow the general contour of the task demonstrations. 
	...
	age: $26 \pm 4$, 8 males and 5 females) and 4 SCI subjects (mean age: $35 \pm 14$, all males, C3-C5 injury
	...
	\noindent{\uline{\textit{Metrics:}}} A number of objective metrics evaluated this study. \textit{Task Completion time} is the amount of time spent accomplishing a task. \textit{Mode Switches} refer to the number of times the subject switched between the various modes of the control interface (Table \ref{tbl:modes}). Mode switch additionally is an indirect measure of the effort put forth by the user. At the
		end of the study, subjective data was gathered via a brief
		questionnaire. To explore subject opinions, users were given
		statements about the assistance system, and asked to rate
		each one of them on a 7-point Likert scale (1 is low, 7 is higher), according to
	...
	Here we report the results of our pilot study. An improvement in task performance with customization is demonstrated, and a number of other interesting observations are noted. Task performance metrics for different assistance levels (denoted by \textit{min}, \textit{max} and \textit{custom} in the plots) and teleoperation (\textit{tel)} are analyzed across different subject groups, tasks and control interfaces. Note that the \textit{custom} assistance level always lies in between (or is equal to) \textit{min} and \textit{max}. 
	...
	Figure \ref{PlotOne} (first column) represents the task completion time (top) and mode switches (bottom) for uninjured and SCI subject groups, averaged across all tasks and interface mapping paradigms. For the \textit{custom} assistance type, task completion times are comparable to those of the \textit{max} assistance type.
	...
	%\vspace*{-0.09cm}
	This  provides insight that the true cost function that the user is optimizing is more complex than a simple time--optimal or minimum--effort cost function. 
	
	\vspace{0.1cm}
	\noindent{\uline{\textit{Task Performance:}}} 
	In Figure \ref{PlotOne} (first column), the difference between task completion times for uninjured and SCI subjects drops steadily from \textit{tel} to \textit{custom} assistance levels. The t-tests revealed that while the difference between uninjured and SCI was statistically significant for both \textit{tel} ($p$ = $5.1e$-$4$) and \textit{min} ($p$ = $6.5e$-$5$), this difference disappeared with the \textit{max} ($p$ = $0.027$) and \textit{custom} ($p$ = $0.096$) assistance levels. That is, with increased assistance, the performance of SCI subjects was comparable to that of uninjured subjects and this was \textit{maximized} under customization. The variance in the data also \textit{decreases} with customized assistance, showing the performance to become more consistent. 
	...
	Figure \ref{PlotOne} (second and third columns) shows how task completion times and number of mode switches change between the first and second task. For uninjured subjects, there was no change in performance over trials, and neither between different assistance levels for either metric. The same was true for SCI subjects with the \textit{exception} of the \textit{custom} level. Specifically SCI subjects saw an improvement in task completion times ($p$ = $7.8e$-$3$) \textit{and} mode switches ($p$ = $8.9e$-$3$) between the first and second tasks for the \textit{custom} assistance level. There is also significant improvement in task completion times ($p$ = $3.2e$-$3$ for Task 1 and $p$ = $8.2e$-$4$ for Task 2) and mode switches ($p$ = $1.6e$-$4$ for Task 1 and $p$ = $4.9e$-$5$ for Task 2) for levels \textit{tel} and \textit{custom}.
	\begin{figure}[t]
		\centering
	...
	\subsection{Relative Change in Parameters during Customization}
	Figure~\ref{RCAP} shows the change in amount of assistance during customization for uninjured and SCI subjects. While SCI subjects on average increased the amount of assistance ($p$\;=\;$0.02$) during the second phase of customization, uninjured subjects chose to reduce the amount of assistance ($p$ = $0.006$). There were no noticeable changes in the customized assistance levels between the \textit{2D} and \textit{3D} interfaces. Injury seems to be the primary reason why subjects choose to change the customized assistance levels and the mapping paradigm seem to have little effect. 
	...
	\begin{figure}[t]
		\centering
		\includegraphics[scale=0.46]{./finalfigures/Figure6.eps}
		\vspace{-0.6cm}
		\caption{Relative change in assistance parameters during customization for Uninjured vs. SCI subjects (left) and 2D vs. 3D subjects (right).}
		\label{RCAP}
	\end{figure}
	...
	Users rated the utility value of the assistance system fairly high (mean = $5.9\pm0.8$) indicating that in general having assistance was favored. The users also thought that the system was able to perceive goals quite accurately (mean = $5.1\pm1.8$) and the inability to estimate human intent was fairly low (mean = $3.1\pm1.1$). The users also felt that they played an important part in accomplishing the task (mean = $5.5\pm1.0$), almost comparable to the contribution from assistance (mean = $5.1\pm1.6$), maybe indicating that they were not prepared to relinquish control altogether (Fig.~\ref{US}).
	\begin{figure}[t]
		%	\centering
		\includegraphics[scale = 0.42, center]{./finalfigures/Figure7.eps}
		\vspace{-0.5cm}
		\caption{User questionnaire responses.User questionnaire responses.}
		\label{US}
	\end{figure}
	\vspace{-0.08cm}
	\subsection{Discussion}
	From our pilot study we saw that compared to pre-defined assistance levels, customization improves task performance and helps to reduce performance differences between uninjured and SCI subjects. Post-experiment surveys also revealed that the users found the customized assistance paradigm to be useful. These results establish a need for customization of assistance levels. Therefore, our next step will be to explore multiple possibilities for building effective and intuitive customization mechanisms (for example, physical interfaces operated by the user) which will suit individual requirements and preferences and to evaluate on a larger end-user population. The results also show that the true cost function that is being optimized is more complex than a simple time-optimal or minimum-effort cost function indicating the need to investigate the exact specification of the true cost function that is being optimized by the human in a shared control system. A more comprehensive user survey will also be administered in which the formalized customization procedure will be evaluated thoroughly.
	%\addtolength{\textheight}{-0.1cm}  
	\section{CONCLUSIONS}\label{CON}
	In this work, we formalize human robot interaction in shared autonomy within the framework of optimal control theory. Furthermore we have also introduced a system for user-driven customization which is presented as a constrained non-linear optimization problem within this framework. Unlike standard optimization problems in which the form of the cost function is predetermined in this work no such assumptions were made. Instead, the end user was allowed to directly perform the optimization procedure. The aim is that this will lead to higher user satisfaction, which is crucial for the acceptance of novel technologies in the assistive domain. An interactive user-driven customization system was also developed to ground the formalism and the results from the pilot study were also presented. Results showed that all subjects were able to converge to a optimal assistance paradigm and an improvement in task performance with customization was also demonstrated.
